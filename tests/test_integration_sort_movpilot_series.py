...
class TestAmazonPattern:
    class TestDefaultDirectory:
        ...
        def test_multiple_files(tmp_path: Path, cwd, mocker: MockerFixture):
            ...
            for file in ["S01E001_the_man_trap.mp4", "S01E002_charlie_x.mp4"]:
                (old_file := (tmp_path / series_name / "1" / file)).write_text(
                    "<sentinel>"
                )
            ...
            for file in ["S02E001_amok_time.mp4", "S02E002_who_mourns_for_adonais.mp4"]:
                (old_file := (tmp_path / series_name / "2" / file)).write_text(
                    "<sentinel>"
                )
                old_files.append(old_file)
    ...
    class TestCustomDirectory:
        ...
        def test_relative_dir(
            ...
            series_name = "firefly"
            (tmp_path / subdir / series_name).mkdir()
            (old_subdir := (tmp_path / subdir / series_name / "1")).mkdir()
            (
                old_file := (
                    tmp_path / subdir / series_name / "1" / "S01E001_serenity.mp4"
                )
            ).write_text("<sentinel>")
            mocker.patch(
